...
export const getMenu = createAsyncThunk("dishes/getMenu", async () => {
    try {
        if (window.location.href.slice(0, 16) !== "http://localhost") {
            const res = (
                await axios.get("https://foodify-akk3.onrender.com/api/dish")
            ).data;
            return res;
        } else {
            const res = (await axios.get("http://localhost:4200/api/dish"))
                .data;
            return res;
        }
    } catch {
...
export const dishId = createAsyncThunk("dish/dishId", async (_id) => {
    try {
        if (window.location.href.slice(0, 16) !== "http://localhost") {
            const res = (
                await axios.get(
                    `https://foodify-akk3.onrender.com/api/dish/${_id}`
                )
            ).data;
            return res;
        } else {
            const res = (
                await axios.get(`http://localhost:4200/api/dish/${_id}`)
            ).data;
            return res;
        }
    } catch {
...
export const getMenuFilter = createAsyncThunk(
    "dishes/getMenuFilter",
    async (payload) => {
        return payload;
    }
);
...
export const searchDate = createAsyncThunk(
    "search/searchDate",
    async (payload) => {
        return payload;
    }
...
export const addDishCart = createAsyncThunk(
    "cart/addDishCart",
    async (payload) => {
        return payload;
    }
);
export const addDishCartPost = createAsyncThunk(
    "cart/addDishCartPost",
    async (payload) => {
        return payload;
    }
);
...
export const deleteAllDish = createAsyncThunk(
    "cart/deleteAllDish",
    async (payload) => {
        return payload;
    }
...
export const orderBy = createAsyncThunk("order/orderBy", async (payload) => {
    return payload;
});

export const sortBy = createAsyncThunk("sort/sortBy", async (payload) => {
    return payload;
});
...
export const getTable = createAsyncThunk("table/getTable", async () => {
    try {
        if (window.location.href.slice(0, 16) !== "http://localhost") {
            const res = (
                await axios.get("https://foodify-akk3.onrender.com/api/table")
            ).data;
            return res;
        } else {
            const res = (await axios.get("http://localhost:4200/api/table"))
                .data;
            return res;
        }
    } catch {
...
export const putTable = createAsyncThunk("table/putTable", async (payload) => {
    try {
        if (window.location.href.slice(0, 16) !== "http://localhost") {
            const id = localStorage.getItem("tableID");
            return await axios.put(
                `https://foodify-akk3.onrender.com/api/table/${id}`,
                payload
            );
        } else {
            const id = localStorage.getItem("tableID");
            return await axios.put(
                `http://localhost:4200/api/table/${id}`,
                payload
            );
        }
    } catch {
...
export const putTable2 = createAsyncThunk("table/putTable", async (payload) => {
    try {
        if (window.location.href.slice(0, 16) !== "http://localhost") {
...
export const addIdTable = createAsyncThunk(
    "table/addDishTable",
    async (payload) => {
        return payload;
    }
...
export const getOrder = createAsyncThunk("order/getOrder", async (payload) => {
    try {
        if (window.location.href.slice(0, 16) !== "http://localhost") {
            const res = (
                await axios.get(
                    `https://foodify-akk3.onrender.com/api/order/${payload}`
                )
            ).data;
            return res;
        } else {
            const res = (
                await axios.get(`http://localhost:4200/api/order/${payload}`)
            ).data;
            return res;
        }
    } catch {
...
export const getActualOrder = createAsyncThunk(
    ...
    async (payload) => {
        try {
            if (window.location.href.slice(0, 16) !== "http://localhost") {
                ...
                const res = (
                    await axios.get(
                        `http://localhost:4200/api/order/${payload}`
                    )
                ).data;
                return res;
            }
...
export const getOrder2 = createAsyncThunk("order/getOrder2", async () => {
    try {
        if (window.location.href.slice(0, 16) !== "http://localhost") {
            const res = (
                await axios.get(`https://foodify-akk3.onrender.com/api/order/`)
            ).data;
            return res;
        } else {
            const res = (await axios.get(`http://localhost:4200/api/order/`))
                .data;
            return res;
        }
    } catch {
...
export const getMetricOrder = createAsyncThunk(
    "metric/getMetricOrder",
    async (payload) => {
        return payload;
    }
);
    ...
    async () => {
        try {
            if (window.location.href.slice(0, 16) !== "http://localhost") {
                const res = (
                    await axios.get(
                        "https://foodify-akk3.onrender.com/api/category"
                    )
                ).data;
                return res;
            } else {
                const res = (
                    await axios.get("http://localhost:4200/api/category")
                ).data;
                return res;
            }
        } catch {
...
export const getReviews = createAsyncThunk("reviews/getReviews", async () => {
    try {
        if (window.location.href.slice(0, 16) !== "http://localhost") {
            const res = (
                await axios.get("https://foodify-akk3.onrender.com/api/review")
            ).data;
            return res;
        } else {
            const res = (await axios.get("http://localhost:4200/api/review"))
                .data;
            return res;
        }
    } catch {
...
export const getWaiters = createAsyncThunk(
    "waiters/getWaiters",
    async (payload) => {
        return payload;
    }
    ...
    async (payload) => {
        try {
            if (window.location.href.slice(0, 16) !== "http://localhost") {
                const res = (
                    await axios.get(
                        `https://foodify-akk3.onrender.com/api/orderid/${payload}`
                    )
                ).data;
                return res;
            } else {
                const res = (
                    await axios.get(
                        `http://localhost:4200/api/orderid/${payload}`
                    )
                ).data;
                return res;
            }
    ...
    extraReducers: (builder) => {
            ...
            for (let i = 0; i < orderCount.length; i++) {
                if (dish.find((e) => e.name === orderCount[i].name)) {
                    for (let j = 0; j < dish.length; j++) {
                        if (dish[j].name === orderCount[i].name)
                            dish[j].quantity++;
                    }
                } else {
                    dish.push({
        ...
        builder.addCase(getMenuFilter.fulfilled, (state, action) => {
            ...
            const filtered =
                action.payload === "all"
                    ? all
                    : all.filter((d) => d.category?.includes(action.payload));
        ...
        builder.addCase(searchOrder.fulfilled, (state, action) => {
            ...
            const filterSearch = search.filter((e) =>
                e.order_number?.includes(action.payload)
            );
        ...
        builder.addCase(searchDate.fulfilled, (state, action) => {
            const search = state.order2Filter;
            const filterSearch = search.filter((e) =>
                e.timestamps?.slice(0, 10).includes(action.payload)
            );
            state.order2 = filterSearch;
        });
        ...
        builder.addCase(getWaiters.fulfilled, (state, action) => {
            ...
            const f =
                action.payload === "all"
                    ? state.order2Filter
                    : all.filter((e) => e.waiter?.includes(action.payload));
            state.order2 = f;
        });
    },
});

export const { clearCart, cleanDetail, clearCartPost, cleanOrderId } =
    menuSlice.actions;
